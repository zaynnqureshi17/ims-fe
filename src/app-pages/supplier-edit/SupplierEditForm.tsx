...
const SupplierEditForm = ({ supplierData }: { supplierData: ISupplier }) => {
  const {
    supplier_id,
    supplier_image_url,
    category,
  ...
  const { mutate: updateSupplier, status: mutationStatus } =
    useUpdateSupplier();
  ...
  const methods = useForm({
    defaultValues: {
      category,
      company_name,
      registration_code: supplierData.registration_code || "",
      pic,
      contact_number: supplierData.contact_number || "",
      email,
      supplier_code: supplierData.supplier_code || "",
      finance_code: supplierData.finance_code || "",
      status: supplierStatus || "",
      area_short: area_short || [],
      supplier_image_url: supplier_image_url || null,
    },
  });

  const { handleSubmit, reset, control } = methods;
  const loading = mutationStatus === "pending";

  const onSubmit = (data: any) => {
    console.log(data);
    updateSupplier(
      { body: data, id: supplier_id },
      {
        onSuccess: () => {
          reset(data); // reset to new values, not empty
