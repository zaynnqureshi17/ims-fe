import { useMutation, useQueryClient } from "@tanstack/react-query";
import { toast } from "react-toastify";

import axiosInstance from "@/services/axiosInstance";
import { ErrorResponseType } from "@/utils/types/api.types";

const onCreateCatalogue = async (body: any) => {
  return await axiosInstance.post(`catalogue`, body);
};

export const useCreateCatalogue = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationKey: ["onCreateCatalogue"],
    mutationFn: ({ body }: any) => onCreateCatalogue(body),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["onGetCatalogue"] });
    },
    onError: (error: ErrorResponseType) => {
      toast.error(error?.data?.message || "Some error occurred");
    },
  });
};
